...
def compute_jigsaw_accuracy(pred_jigsaw, targets_jigsaw):
    """
    Compute accuracy for the Jigsaw task.

    Args:
        pred_jigsaw (torch.Tensor): Logits of shape [N, 196]
        targets_jigsaw (torch.Tensor): True patch indices of shape [N]

    Returns:
        float: Accuracy in percentage
    """
    predicted_positions = pred_jigsaw.argmax(dim=1)  # Get predicted indices
    accuracy = (predicted_positions == targets_jigsaw).float().mean().item() * 100
    return accuracy
...
def evaluate(data_loader, model, device):
    ...
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    print('* Acc@1 {top1.global_avg:.3f} Acc@5 {top5.global_avg:.3f} Jigsaw Acc {jigsaw.global_avg:.3f} loss {losses.global_avg:.3f}'
          .format(top1=metric_logger.acc1, top5=metric_logger.acc5, jigsaw=metric_logger.jigsaw_acc, losses=metric_logger.loss))
